...
flags = tf.app.flags
flags.DEFINE_string('data_dir', '/root/DB/ICDAR2015_Incidental/', 'Root directory to raw pet dataset, like /startdt_data/HDA_Dataset_V1.3/VOC_fmt_training_fisheye')
flags.DEFINE_string('output_dir', '/root/DB/ICDAR2015_Incidental/tfrecord/', 'Path to directory to output TFRecords, like models/hda_cam_person_fisheye')
FLAGS = flags.FLAGS
...
def create_tf_record(output_path, data_dir):
    ...
    """
    # Train_tfrecord
    writer_train = tf.python_io.TFRecordWriter(output_path + "train_IC15_mask.record")
    ...
    for n, i in enumerate(train_list):
        ...
        img_file = data_dir + "train_mask/%s.jpg" % (i)
    ...
    # Valid_tfrecord
    writer_val = tf.python_io.TFRecordWriter(output_path + "val_IC15_mask.record")
    ...
    for n, i in enumerate(val_list):
        ...
        img_file = data_dir + "test_mask/%s.jpg" % (i)
